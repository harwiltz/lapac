...
class Decoder(nn.Module):
    ...
    def forward(self, latents):
        ...
        data = torch.reshape(latents, (-1, *latents.shape[-3:]))
        out = self._conv1(data)
        out = F.relu(out)
        out = self._conv2(out)
        out = F.relu(out)
        ...
        out = out.reshape(latents.shape[:-3] + out.shape[-3:])
...
class ModelNetwork(nn.Module):
    def __init__(
        ...
        # z^1_1 ~ p(z^1_1) = N(0, I)
        self._z1_first_prior = dtb.normal.Normal(loc=torch.zeros(self._z1_size), scale=1.0)
        # z^2_1 ~ p(z^2_1 | z^1_1)
    ...
    def compute_loss(self, experience):
        ...
        z1_samples = torch.cat(z1_posterior_samples, axis=-1)
        z2_samples = torch.cat(z2_posterior_samples, axis=-1)
        ...
        reconstruction_loss = -reconstuction_logprobs.mean()
    ...
    def z1_prior_dists_given_samples(self, z1_samples, z2_samples, actions, step_types):
        ...
        sequence_length = actions.shape[0] - 1
        first_z1_dist = self._z1_first_prior
        z1_dists.append(first_z1_dist)
        for t in range(1, sequence_length + 1):
